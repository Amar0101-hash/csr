...
def main() -> int:
    ...
    if cmd == "build":
        from graph_rag.dataingestion.build_graph import build
        build()

    elif cmd == "enrich":
        from graph_rag.dataingestion.enrich import enrich
        enrich()

    elif cmd == "template":
        from graph_rag.dataingestion.template_graph import build_template, coverage
        build_template()
        coverage()
        ...
        from graph_rag.fill_section import fill
        fill(arg or "6.2.1")

    elif cmd == "report":
        from graph_rag.generate import generate_report
        limit = int(arg) if arg.isdigit() else None
        generate_report(limit=limit)

    elif cmd == "trace":
        import json
        from graph_rag.trace_view import section_view
        print(json.dumps(section_view(arg or "6.3.5"), indent=2))

    elif cmd == "trace-export":
        from graph_rag.trace_view import export_all
        d = export_all()
        print(f"wrote per-section trace views + coverage.json to {d}")

    elif cmd == "diff":
        from graph_rag.compare_reports import compare
        compare()
