"""In-memory knowledge graph over source chunks and clinical entities (networkx).

Shared substrate for graph-aware retrieval without a database: nodes are chunks
and entities; edges connect a chunk to every entity it mentions. Retrieval uses
it to expand a vector/FTS hit set with other chunks that share the same clinical
entities — crucial for CSR consistency, where an endpoint is *defined* in the SAP,
*described* in the Protocol, and *measured* in a TFL table. Consumed by hybrid_rag
(and available to a future NetworkX-based graph_rag, per the AWS plan).
